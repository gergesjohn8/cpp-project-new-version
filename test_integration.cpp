...
int main() {
    ...
    const double tol_strict = 1e-6;
    // Looser tolerance for singular functions (f3, f4) and Monte Carlo
    const double tol_singular = 1e-4;
    const double tol_monte_carlo = 1e-2;  // Monte Carlo has high variance
    ...
    deterministic_solvers.emplace_back(std::make_unique<TrapezoidSolver>(20000));
    deterministic_solvers.emplace_back(std::make_unique<SimpsonSolver>(20000));
    deterministic_solvers.emplace_back(std::make_unique<WeddleSolver>(20000));
    ...
    for (std::size_t i = 0; i < deterministic_solvers.size(); ++i) {
        ...
        bool passed = approx_equal(val, true_f3, tol_singular);
        std::cout << "  " << det_solver_names[i] << ": " << val
                  << (passed ? " [PASS]" : " [FAIL]") << "\n";
        tests_total++;
        if (passed) tests_passed++;
    ...
    
    {
        ...
        bool passed = approx_equal(val, true_f3, tol_monte_carlo);
        std::cout << "  Monte Carlo: " << val
                  << (passed ? " [PASS]" : " [FAIL - expected for MC]") << "\n";
        tests_total++;
        if (passed) tests_passed++;
    ...
    for (std::size_t i = 0; i < deterministic_solvers.size(); ++i) {
        ...
        bool passed = approx_equal(val, true_f4, tol_singular);
        std::cout << "  " << det_solver_names[i] << ": " << val
                  << (passed ? " [PASS]" : " [FAIL]") << "\n";
        tests_total++;
        if (passed) tests_passed++;
    ...
    
    {
        ...
        bool passed = approx_equal(val, true_f4, tol_monte_carlo);
        std::cout << "  Monte Carlo: " << val
                  << (passed ? " [PASS]" : " [FAIL - expected for MC]") << "\n";
        tests_total++;
        if (passed) tests_passed++;
